...
function RegisterForm() {
  ...
  // Pre-select provider if from CTA
  React.useEffect(() => {
    if (searchParams.get('provider') === 'true') {
      setRole('provider');
    }
  }, [searchParams]);
  ...
  const handleEmailRegister = async (e: React.FormEvent) => {
    ...
    showToast(`Welcome to Paws & Co., ${firstName}! 🎉`, 'success');
  ...
  return (
    <>
      <GoogleAuthModal isOpen={showGoogleModal} onClose={() => setShowGoogleModal(false)} onSuccess={handleGoogleSuccess} />
        ...
        <div className="w-full max-w-[460px]">
          <div className="bg-white rounded-2xl p-12 sm:p-10 border border-[#F0E4D8] shadow-lg">
            <div className="text-center mb-8">
              ...
              <h2 className="text-2xl font-heading text-[#2C3E50] mb-1">Join Paws & Co.</h2>
              <p className="text-sm text-gray-500">Create your account and get started today</p>
            </div>
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-3 bg-white border-2 border-gray-200 text-gray-700 font-semibold py-3.5 px-6 rounded-full hover:border-gray-400 hover:shadow-md transition-all"
            >
            ...
            <form onSubmit={handleEmailRegister}>
              <div className="grid grid-cols-2 gap-4">
                <div className="mb-5">
                  <label className="block text-sm font-semibold text-[#2C3E50] mb-2">First Name</label>
                ...
                <div className="mb-5">
                  <label className="block text-sm font-semibold text-[#2C3E50] mb-2">Last Name</label>
            ...
            <p className="text-center mt-6 text-sm text-gray-500">
              Already have an account? <Link href="/login" className="font-semibold text-[#E86A33]">Sign In</Link>
            </p>
          </div>
        </div>
      </div>
...
export default function RegisterPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><div className="w-10 h-10 border-3 border-[#F0E4D8] border-t-[#E86A33] rounded-full animate-spin" /></div>}>
      <RegisterForm />
    </Suspense>
  );
}
